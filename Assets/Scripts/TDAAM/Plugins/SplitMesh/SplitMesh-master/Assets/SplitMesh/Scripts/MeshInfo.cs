...
/// </summary>
    public class MeshInfo
    {
        ...
        public Mesh CombineMesh(params MeshInfo[] info)
        {
            CombineInstance[] coms = new CombineInstance[info.Length];
            for (int i = 0; i < info.Length; i++)
            {
                coms[i].mesh = info[i].GetMesh();
				//coms[i].mesh.indexFormat = UnityEngine.Rendering.IndexFormat.UInt32;
				coms[i].transform = info[i].localToWorldMatrix;
            }
            Mesh mesh = new Mesh();
