...
namespace TDAAM.Measure
{
    ...
    public class DistanceMeasure_surface : MonoBehaviour
    {
        public event Action<bool> OnMeasureCompleted;
        public event Func<Vector3, Vector3, int, float> OnAddPointAction;
        public event Action<int, Vector3, Vector3, int> OnChangedPointAction;
        public event Action<int> OnRemovePointAction;
        ...
        private void MouseInput()
        {
            ...
            if (Physics.Raycast(Camera.main.ScreenPointToRay(Input.mousePosition), out RaycastHit mouseHit))
            {
                ...
                else if (Input.GetMouseButtonUp(0) || isCreate)
                {
                    ...
                    if (doubleClickTimer > doubleClickTime || isCreate)
                    {
                        ...
                        if (mousePoints.Count >= 2)
                        {
                            ...
                            if (OnAddPointAction != null)
                            {
                                TotalLength += OnAddPointAction.Invoke(mousePoints[mousePoints.Count - 2], mousePoints[mousePoints.Count - 1], 0);
                ...
                else if (Input.GetMouseButtonUp(1))
                {
                    ...
                    OnRemovePointAction?.Invoke(mousePoints.Count);
                    //lineRenderer.positionCount = mousePoints.Count;
                }
                if (mousePoints.Count > 0)
                {
                    OnChangedPointAction(0, mousePoints[mousePoints.Count - 1], tempPoint, 1);
                ...
                else OnChangedPointAction(0, Vector3.zero, Vector3.zero, 1);
        ...
        void OnGUI()
        {
            ...
            if (mousePoints.Count > 0)
            {
                ...
                {
                    //因为表面测量会对Mesh进行三角形遍历，所以不Update进行表面长度测量,所以这里未确定点的长度就不计算了
                ...
                if (isMeasureCompleted)
                {
                    ...
                    rect = new Rect(new Vector2(screenPoint.x, Screen.height - screenPoint.y - fontStyle.fontSize), new Vector2(fontStyle.fontSize, fontStyle.fontSize));
                    GUI.Label(rect, "空间总长: " + distanceTwoPoints[distanceTwoPoints.Count - 1].ToString("f2") + "米", fontStyle);
                }
                fontStyle.normal.textColor = uiColor_Confirm;
        ...
        public void OnDoubleClick(bool isStop = false)
        {
            //这一步为了设置最后一下就是最后点击鼠标的地方
            tempPoint = mousePoints[mousePoints.Count - 1];
            tempDistance = 0;

            OnChangedPointAction(0, Vector3.zero, Vector3.zero, 1);
            isMeasureCompleted = true;
            OnMeasureCompleted?.Invoke(isStop);
        }
    }
}